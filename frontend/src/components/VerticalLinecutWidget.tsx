...
const VerticalLinecutWidget: React.FC<VerticalLinecutWidgetProps> = ({
  ...
  const [minQXValue, maxQXValue] = React.useMemo(() => {
    ...
    // Extract values from middle row - for vertical linecuts, we're interested in qX values
    // which vary across the columns
    ...
    if (qXMatrix[middleRow]) {
      for (let x = 0; x < qXMatrix[middleRow].length; x++) {
        if (qXMatrix[middleRow][x] !== undefined) {
          minVal = Math.min(minVal, qXMatrix[middleRow][x]);
          maxVal = Math.max(maxVal, qXMatrix[middleRow][x]);
        }
      }
    }
