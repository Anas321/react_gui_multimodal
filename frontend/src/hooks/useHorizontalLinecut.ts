...
/**
 * Custom hook for managing horizontal linecuts based on q-values using full q-matrices
 *
 * This hook provides functionality to create, manipulate, and visualize horizontal linecuts
 * on scattering images working in q-space (reciprocal space) with full 2D q-matrices.
 *
 * @param imageHeight - Height of the detector image in pixels
 * @param qYMatrix - 2D matrix of q-values, mapping each pixel to its q-value
 * @returns Object with linecut data and management functions
 */
export default function useHorizontalLinecut(
    imageHeight: number,
    qYMatrix: number[][] // Now expecting a full 2D matrix
) {
  // State for storing the linecut definitions (position, width, colors, etc.)
  const [horizontalLinecuts, setHorizontalLinecuts] = useState<Linecut[]>([]);

  /**
   * Find pixels where q_y values are close to the target q-value
   ...
   */
  const findQYPositions = useCallback((
    targetQ: number,
    ...
    if (!qYMatrix || qYMatrix.length === 0) return positions;

    // Find a row where q-values are close to target (typically q-values are similar across a row)
    // For horizontal linecuts, we need to find rows where qY is close to our target
    for (let y = 0; y < qYMatrix.length; y++) {
      // Check the first element in each row (assuming q-values are relatively consistent across a row)
      if (qYMatrix[y] && Math.abs(qYMatrix[y][0] - targetQ) < tolerance) {
        // We've found a row with the right q-value, return its index
        return [[y, 0]]; // Just return the row index with column 0
      }
    }
  ...
  /**
   * Finds the pixel row that best corresponds to a q-value
   * @param targetQ - Q-value to find
   * @returns Row index that best corresponds to the given q-value
   */
  const findClosestPixelForQValue = useCallback((
    targetQ: number
  ): number => {
    // Handle the case where qYMatrix is empty with a reasonable default
    if (!qYMatrix || qYMatrix.length === 0 || !qYMatrix[0]) {
      return Math.floor((imageHeight - 1) / 2); // Default to middle of image
    }

    // Find positions matching the q-value
    const positions = findQYPositions(targetQ);

    // If found, return the row index
    if (positions.length > 0) {
      return positions[0][0]; // Return the row index of the first match
    }

    // If no exact match, find the closest row
    let closestRow = 0;
    let smallestDifference = Infinity;

    for (let y = 0; y < qYMatrix.length; y++) {
      if (qYMatrix[y] && qYMatrix[y][0] !== undefined) {
        const difference = Math.abs(qYMatrix[y][0] - targetQ);
        if (difference < smallestDifference) {
          smallestDifference = difference;
          closestRow = y;
        }
    ...
    return closestRow;
  }, [qYMatrix, findQYPositions, imageHeight]);


  /**
   * Creates a new horizontal linecut
   */
  const addHorizontalLinecut = useCallback(throttle(() => {
    // Find the next available ID for the new linecut
    const existingIds = horizontalLinecuts.map((linecut) => linecut.id);
