...
from src.get_single_image_array_and_name import get_single_image_array_and_name_async
...
async def create_raw_data_overview():
    ...
    max_intensities = np.zeros(num_of_files, dtype=float)
    avg_intensities = np.zeros(num_of_files, dtype=float)
    image_names = [""] * num_of_files

    # Process in batches to control concurrency
    batch_size = 20  # Adjust based on your server and API capabilities
    processed_count = 0

    for batch_start in range(0, num_of_files, batch_size):
        batch_end = min(batch_start + batch_size, num_of_files)
        batch_indices = range(batch_start, batch_end)

        # Create tasks for this batch
        tasks = [
            process_single_image_async(
                i,
                all_files_uris[i],
                mask_detector,
                tiled_uri,
        ...
        for result in results:
            index, max_intensity, avg_intensity, image_name, success = result
    ...
    # Prepare serializable data and return
    serializable_data = {
        "max_intensities": max_intensities.tolist(),
        "avg_intensities": avg_intensities.tolist(),
        "image_names": image_names,
...
async def process_single_image_async(
    index, uri, mask_detector, tiled_uri, data_local_path, DEV_MODE
):
    try:
        t0 = time.time()

        # Get the image array and name
        image_array, image_name = await get_single_image_array_and_name_async(
            uri, mask_detector, tiled_uri, data_local_path, DEV_MODE
        )
        t1 = time.time()
